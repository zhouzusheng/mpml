 ...
 * encountered.
 */
ac_error_code ac_output_list_add(ac_list *self, ac_offset offset, int data)
{
	ac_output *new_item;
...
ac_error_code ac_output_list_add_list(ac_list *self, ac_list *other)
{
	...
	while (list_item) {
		...
		if (ac_output_list_add(self, item->offset, item->data) !=
		    AC_SUCCESS) {
			return AC_FAILURE;
		}
...
ac_error_code ac_cb_output(ac_result_callback result_cb,
						   ...
						   int* index)
{
	ac_result new_item = {.start = index[start], .end = index[end], .data = data};
 ...
 */
ac_error_code
ac_state_free(ac_state *self, ac_list *children)
{
	if (!self) {
		return AC_FAILURE;
	}
	...
	ac_output_list_free(self->outputs, ac_list_free_keep_item);
...
ac_index ac_index_new(void)
{
	...
	self->next_id = 0;
	if (!(self->state_0 = ac_state_new(self->next_id++))) {
		return NULL;
	}
...
ac_error_code ac_index_root_free(ac_index self) {
	ac_list *queue = NULL;
	ac_state *state = NULL;
	ac_error_code result = AC_SUCCESS;
	...
	while (state) {
		...
		if (ac_state_free(state, queue) != AC_SUCCESS) {
			result = AC_FAILURE;
		}
...
{
	ac_error_code result = ac_index_root_free(self);
	if(self)
		free(self);
	return result;
}

ac_error_code duplicate_states(ac_index self, ac_state **states, int count) {
	ac_list * list;
	ac_goto *goto_item;
	...
	while (state) {
		...
		if(list) {
			...
			while (list_item) {
				goto_item = list_item->item;
				if(states[goto_item->state->id] == NULL) {
					ac_list_add(queue, goto_item->state);
				}
				list_item = list_item->next;
			}
		}
		if (state->failure && states[state->failure->id] == NULL) {
			ac_list_add(queue, state->failure);
		}
		state = ac_state_queue_get(queue);
	}
	ac_state_queue_free(queue); // The queue should be empty.
...
void ac_state_save(ac_state* state, FILE* fp) {
	int len;
	ac_list_item* list_item;
	int data = htonl(state->id);

	//write id
	fwrite(&data, sizeof(data), 1, fp);

	//write goto
	len = state->gotos ? ac_list_size(state->gotos) : 0;
	data = htonl(len);
	fwrite(&data, sizeof(data), 1, fp);
	if(len > 0) {
		list_item = state->gotos->first;
		while(list_item) {
			ac_goto* gotoitem = list_item->item;
			fwrite(&gotoitem->symbol, sizeof(char), 1, fp);
			data = htonl(gotoitem->state->id);
			fwrite(&data, sizeof(data), 1, fp);
			list_item = list_item->next;
		}
	}
	//write outputs
	len = state->outputs ? ac_list_size(state->outputs) : 0;
	data = htonl(len);
	fwrite(&data, sizeof(data), 1, fp);
	if(len > 0) {
		list_item = state->outputs->first;
		while(list_item) {
			ac_output* outputitem = list_item->item;
			data = htonl(outputitem->offset);
			fwrite(&data, sizeof(data), 1, fp);
			data = htonl(outputitem->data);
			fwrite(&data, sizeof(data), 1, fp);
			list_item = list_item->next;
		}
	}
	...
	//write extra_outputs
	len = state->extra_outputs ? ac_list_size(state->extra_outputs) : 0;
	data = htonl(len);
	fwrite(&data, sizeof(data), 1, fp);
	if(len > 0) {
		list_item = state->extra_outputs->first;
		while(list_item) {
			ac_output* outputitem = list_item->item;
			data = htonl(outputitem->offset);
			fwrite(&data, sizeof(data), 1, fp);
...
ac_error_code ac_index_save(ac_index self, PyObject* file) {
	FILE* fp;
	int data = 0;
	int state_length;
	...
	//write index_state
	data = htonl(self->index_state);
	fwrite(&data, sizeof(data), 1, fp);

	//write states
	state_length = self->next_id;
	ac_state **states = calloc(state_length, sizeof(ac_state *));
	duplicate_states(self, states, state_length);
	data = htonl(state_length);
	fwrite(&data, sizeof(data), 1, fp);
	for(int i = 0; i < state_length; i++) {
		ac_state_save(states[i], fp);
	}
	free(states);
	fclose(fp);
	return AC_SUCCESS;
...
ac_error_code ac_index_load(ac_index self, PyObject* file) {
	FILE* fp;
	int data = 0;
	ac_state **states;
	int state_length;
	...
	ac_index_root_free(self);
	self->index_state = ntohl(data);
	if(fread(&data, sizeof(data),1, fp) <=0) {
		//todo
		fclose(fp);
		return AC_FAILURE;
	}
	state_length = ntohl(data);
	self->next_id = state_length;
	states = malloc(state_length* sizeof(ac_state *));
	for(int i = 0; i < state_length; i++) {
		//id
		ac_state * state = states[i] = ac_state_new(i);
		if(i == 0) {
			self->state_0 = state;
		}
	}
	for(int i = 0; i < state_length; i++) {
		...
		if(len > 0) {
			...
			for(int j = 0; j < len; j++) {
				...
				new_item = malloc(sizeof(ac_goto));
				new_item->symbol = ch;
				new_item->state = states[id];
				ac_list_add(state->gotos, new_item);
			}
		}
		data = -1;
		fread(&data, sizeof(data),1, fp);
		...
		if(len > 0) {
			//read output table
			for(int j = 0; j < len; j++) {
				ac_output *new_item = malloc(sizeof(ac_output));
				data = -1;
				fread(&data, sizeof(data),1, fp);
				new_item->offset =  ntohl(data);

				data = -1;
				fread(&data, sizeof(data),1, fp);
				new_item-> data =  ntohl(data);
				ac_list_add(state->outputs, new_item);
			}
		}
		data = -1;
		fread(&data, sizeof(data),1, fp);
		...
		if(len > 0) {
			...
			for(int j = 0; j < len; j++) {
				...
				fread(&data, sizeof(data),1, fp);
				new_item-> data =  ntohl(data);
				ac_list_add(state->extra_outputs, new_item);
			}
		}
		data = -1;
		fread(&data, sizeof(data),1, fp);
...
ac_error_code
ac_index_enter(ac_index self, char *keyword, ac_offset size, int data)
{
	...
	while (j < size) {
		if (!(new_state = ac_state_new(self->next_id++))) {
			return AC_FAILURE;
		}
	...
	if (ac_output_list_add(state->outputs, size, data) != AC_SUCCESS) {
		return AC_FAILURE;
	}
...
ac_error_code ac_utf_8_index(const char *str, int len,  int** index) {
	const unsigned char* data =  (const unsigned char*)str;
	int* idx = NULL;
	if(index == NULL) {
		return AC_FAILURE;
	}
	idx = *index = malloc(sizeof(int) * (len+1));
	int pos = 0;
	int count = 0;
	while(pos < len) {
                                ...
                                void *result_cb_data)
{
	...
	if(ac_utf_8_index(phrase, size, &index) == AC_FAILURE){
		return AC_FAILURE;
	}
	...
	for (; j < size; ++j) {
		...
		if (ac_cb_outputs(
		        result_cb, result_cb_data, state->outputs, j, index) !=
		    AC_SUCCESS) {
			free(index);
			return AC_FAILURE;
		};
		...
		if (ac_cb_outputs(
		        result_cb, result_cb_data, state->extra_outputs, j, index) !=
		    AC_SUCCESS) {
			free(index);
			return AC_FAILURE;
		};
