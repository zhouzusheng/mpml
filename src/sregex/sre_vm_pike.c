...
SRE_API void
sre_vm_pike_reset_ctx(sre_vm_pike_ctx_t *ctx) {
	ctx->processed_bytes = 0;
	ctx->pending_ovector = NULL;
	if (ctx->current_threads) {
		sre_vm_pike_clear_thread_list(ctx, ctx->current_threads);
	}
    ctx->current_threads = sre_vm_pike_thread_list_create(ctx->pool);
	if (ctx->next_threads) {
		sre_vm_pike_clear_thread_list(ctx, ctx->next_threads);
	}
    ctx->next_threads = sre_vm_pike_thread_list_create(ctx->pool);
...
sre_vm_pike_exec(sre_vm_pike_ctx_t *ctx, sre_char *input, size_t size,
    unsigned eof, sre_int_t **pending_matched, pike_callback callback, void* userData)
{
    sre_char                  *sp, *last, *p;
    sre_int_t                  rc;
    sre_uint_t                 i;
...
matched:
				...
				//
				if (callback) {
					sre_vm_pike_prepare_matched_captures(ctx, matched,
						ctx->ovector, 0);
					callback(matched->regex_id, ctx->ovector[0], ctx->ovector[1], userData);
				}
